...
```{r}
data <- oil_funding %>%
  mutate(Lagged_renewable_watts_per_capita = lag(Renewable_watts_per_capita, n = 1))

```

```{r}
ggplot(data, aes(x = year)) +
  geom_line(aes(y = disbursement), color = "blue", size = 1) +
  geom_line(aes(x = year, y = Lagged_renewable_watts_per_capita), color = "red", size = 1) +
  labs(title = "Lag Analysis", x = "Year", y = "Values")
...
```{r}
energy_aid_indicators_renewable_watts_relevant_topics_graph <- energy_aid_indicators_renewable_watts_relevant_topics %>%
  group_by(year, topic) %>%
  summarise(topic_renewable_watts_per_capita = sum(Renewable_watts_per_capita),
            recipient_name = first(recipient_name),
            disbursement = first(disbursement),
            sum_disbursement = sum(sum_disbursement))




  ggplot(data = energy_aid_indicators_renewable_watts_relevant_topics, aes(x = year, y = Renewable_watts_per_capita, color = topic)) +
    geom_point() +
  theme_minimal()
```
