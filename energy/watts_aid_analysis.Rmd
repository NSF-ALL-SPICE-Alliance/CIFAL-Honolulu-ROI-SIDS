...
```{r}
#all_sids_aid_watts_unstandardized
```
...
ggplot(all_sids_aid_watts_unstandardized, aes(x=year)) +
  
  geom_bar(aes(y=sum_aid), stat="identity", size=.1, fill=temperatureColor, color="black", alpha=.4) + 
  geom_line( aes(y=sum_watts / coeff), size=2, color=priceColor) +
  ...
  scale_y_continuous(
    ...
    # Add a second axis and specify its features
    sec.axis = sec_axis(~.*coeff, name="Watts")
  ) + 
